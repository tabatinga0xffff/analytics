defmodule PlausibleWeb.Components.Billing.PlanBox do
  ...
  defp checkout(assigns) do
    ...
    usage_within_limits = {:error, {:over_plan_limits, [:reason_one, :reason_two]}}
    ...
    {checkout_disabled, disabled_message, tooltip_fn} =
    ...
    assigns =
      ...
      |> assign(:disabled_tooltip_fn, tooltip_fn)
      |> assign(:confirm_message, losing_features_message(features_to_lose))

    IO.inspect(tooltip_fn, label: :tooltip_fn)
    ...
    >
      <%= if is_function(@disabled_tooltip_fn, 1) do %>
        <div x-data="{sticky: false, hovered: false}" class="relative">
          <p
  ...
  def get_tooltip_fn({:error, {:over_plan_limits, limits}}) do
    fn assigns ->
      ...
      ~H"""
      The following limits have been exceeded:<br /><br />
      <p :for={limit <- @limits}>
