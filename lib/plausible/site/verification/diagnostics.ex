defmodule Plausible.Site.Verification.Diagnostics do
  ...
  def diagnostics_to_user_feedback(%__MODULE__{could_not_fetch_body: :nxdomain, service_error: e1}) when is_nil(e1)  do
    {:error, "We could not resolve your website via DNS"}
  end

  def diagnostics_to_user_feedback(%__MODULE__{could_not_fetch_body: e1, service_error: e2}) when e1 != false and not is_nil(e2)  do
  ...
  def diagnostics_to_user_feedback(%__MODULE__{snippets_found_in_head: 0, snippets_found_in_body: 0, plausible_installed?: false}) do
